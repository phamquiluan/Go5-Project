...
def crop_box(image, box):
    ...
    cropped_image = image.copy()[box['xmin']:box['xmax'], box['ymin']: box['ymax'], :]

    return cropped_image

def readtext(model, config, org_image, boxes_list):
    device = 'cpu' if config.cuda < 0 else f'cuda:{config.cuda}'
    charset = CharsetMapper(filename=config.dataset_charset_path,
                            max_length=config.dataset_max_length + 1)
    texts = []

    for box in boxes_list:
        image = crop_box(org_image, box)
        image = preprocess(image, config.dataset_image_width, config.dataset_image_height)
        image = image.to(device)
        res = model(image)
        pt_text, _, __ = postprocess(res, charset, config.model_eval)

        texts.append(([box['xmin'], box['xmax'], box['ymin'], box['ymax']], pt_text))
