...
def get_rec(img):
    ...
    contours, hierarchy = cv2.findContours(img, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE) # img, hierarchy retrival mode, contour approx. method
...
def extract_lines(image, scale=20, erode_iters=1, dilate_iters=2, show=True):
    ...
    binary = cv2.adaptiveThreshold(~gray, 255,  # ~取反，很重要，使二值化后的图片是黑底白字
                                   cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY, 15, -10) # block_size, C
...
def process_single_image(image, show=True, save=False, scale=20, erode_iters=1, dilate_iters=2):
    '''
    输入单个图像路径
    1. 提取表格线
    2. 得到横线、竖线的交点
    ...
    draw_bbox(image, bounding_boxs, save=save, show=show)
    ...
    return bounding_boxs # [...., [[x_max, y_max], [x_min, y_min]], ....]


if __name__ == '__main__':
    ROOT_DIR= Path('table_recognition/test_imgs') # ADJUST THE PATH
    ...
    for img in progress(imgs_list):
        ...
        input_image = cv2.imread(str(img), 1)
        cell_boxes = process_single_image(input_image, show=True, save=False)
